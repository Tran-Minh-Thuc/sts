...
<form action="update-user-ratting/{{ $trans->id }}" method="post" enctype="multipart/form-data">
    ...
    <div id="{{ $key }}" class="row tab-content">
        <input style="display: none;" id="self_{{ $parent->id }}" type="text" class="form-control border-0 py-3" name="self_score_{{ $parent->id }}" value="{{ $parent->self_score }}" placeholder="0">
        <input style="display: none;" id="class_{{ $parent->id }}" type="text" class="form-control border-0 py-3" name="class_score_{{ $parent->id }}" value="{{ $parent->class_score }}" placeholder="0">
        <div class="col-12">
            <div class="card">
    ...
    @endforeach
    <div style="padding-top: 25px;" class="text-center mx-auto pb-5  " style="max-width: 600px;   ">
        <button type="submit" style="background-color:#0d6efd; border: none;" href="" class="btn btn-secondary text-white px-5 py-3 rounded-pill">Lưu và tiếp tục</button>
    </div>
</form>
@endif
@endsection
<script>
    ...
    function countSumPoint(type, idChild, idParent) {
        var id_child_score = type + "_" + idChild;
        var child_score = document.getElementById(id_child_score);
        var id_parent_max = "max_" + idParent;
        var parent_max = document.getElementById(id_parent_max).innerHTML.replace(/\D/g, '');
        var childs = document.getElementsByClassName(type + "_" + idParent);
        var parent = document.getElementById(type + "_" + idParent);
        sum = 0;
        for (let i = 0; i < childs.length; i++) {
